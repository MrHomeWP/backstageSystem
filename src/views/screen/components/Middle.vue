<template>
  <div class="middleBox">
    ...
    <div class="mapBox">
      ...
      <div class="subheading">景区实时客流量</div>
      <!-- echarts地图 -->
      <div class="echartsMapBox">
        <div ref="RefMap" class="echartsMap"></div>
      </div>
    </div>
    <!-- 未来30天游客量趋势图 -->
    <div class="tendencyBox">
      <div class="tendency">
        <p>未来30天游客量趋势图</p>
        <p class="img_line"></p>
        <!-- 折线图 -->
...
onMounted(() => {
  //渲染地图echartsS
  let ChartMap = echarts.init(RefMap.value);
  const planePath =
  ...
  //渲染折线图
  let ChartLineChart = echarts.init(RefLineChart.value);
    ...
    series: [
      {
        ...
        areaStyle: {
          ...
          color: new echarts.graphic.LinearGradient(0, 0, 0, 1, [
            {
              offset: 0,
              color: "rgba(255, 183, 0,.6)",
            },
            {
              offset: 0.3,
              color: "rgba(255, 183, 0,.4)",
            },
            {
              offset: 1,
              color: "rgba(255, 183, 0,0.05)",
            },
          ]),
        },
        data: data,
  ...
  ChartLineChart.setOption(option2);
  onBeforeUnmount(() => {
    // 在组件销毁前执行清除数据和销毁echarts实例的操作
    ChartMap.dispose();
    ChartLineChart.dispose();
  });
});
</script>
